...
import { Pause, Play, Download, Redo, Undo, Loader2 } from "lucide-react";
            ...
            <div className="relative flex flex-1 items-center justify-center">
                {!isReady && (
                    <div className="absolute inset-0 z-10 flex items-center justify-center">
                        <Badge variant={"outline"} className="gap-2 bg-background/90 px-3 py-1.5 text-sm text-muted-foreground shadow-sm">
                ...
                <div 
                    ref={containerRef}
                    className={cn("w-full cursor-pointer transition-opacity duration-200", !isReady && "opacity-0")}
                />
            </div>
            ...
            <div className="flex flex-col items-center p-6">
                <div className="grid w-full grid-cols-3">
                    ...
                    <div className="flex items-center justify-center gap-3">
                        <Button variant={"ghost"} size={"icon-lg"} className="flex-col" onClick={() => seekBackward(10)} disabled={!isReady}>
                            <Undo className="size-4 -mb-1"/>
                            <span className="text-[10px] font-medium">10</span>
                        </Button>
                        <Button className="rounded-full" variant={"default"} size={"icon-lg"} onClick={togglePlayPause}>
                            {isPlaying ? (
                        ...
                        <Button variant={"ghost"} size={"icon-lg"} className="flex-col" onClick={() => seekForward(10)} disabled={!isReady}>
